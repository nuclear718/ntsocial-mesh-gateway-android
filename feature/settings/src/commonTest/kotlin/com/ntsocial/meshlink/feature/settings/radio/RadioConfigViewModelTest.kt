...
import com.ntsocial.meshlink.core.domain.usecase.settings.ExportProfileUseCase
import com.ntsocial.meshlink.core.domain.usecase.settings.ExportSecurityConfigUseCase
import com.ntsocial.meshlink.core.domain.usecase.settings.ImportProfileUseCase
...
class RadioConfigViewModelTest {
    ...
    fun setUp() {
        ...
        every { serviceRepository.connectionState } returns
            MutableStateFlow(com.ntsocial.meshlink.core.model.ConnectionState.Connected)

        every { mqttManager.mqttConnectionState } returns
            MutableStateFlow(com.ntsocial.meshlink.core.model.MqttConnectionState.Inactive)
    ...
    fun `setFixedPosition calls useCase`() = runTest {
        ...
        val pos = com.ntsocial.meshlink.core.model.Position(latitude = 1.0, longitude = 2.0, altitude = 0)
    ...
    fun `processPacketResponse updates state on various results`() = runTest {
        ...
        // Error
        every { processRadioResponseUseCase(any(), 123, any()) } returns
            RadioResponseResult.Error(com.ntsocial.meshlink.core.resources.UiText.DynamicString("Fail"))
        packetFlow.emit(MeshPacket())
