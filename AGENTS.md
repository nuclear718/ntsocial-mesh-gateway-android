...
- **Current Status:** Gateway v1 is implemented and has both prior connected-radio queue validation and current three-phone parent-App interoperability validation: protected Provider snapshots, single-use command capabilities, explicit command/event IPC, canonical NTsocial channel provisioning, and an in-memory envelope cache are concrete code. RF scheduler expansion, node policy, persistent/reliable delivery, MeshCore transport, and remote RF-reception verification remain follow-up work.
...
- **Local Build Status (2026-07-17):** The expanded local baseline `spotlessApply spotlessCheck detekt assembleDebug test allTests kmpSmokeCompile :app:lintFdroidDebug :app:lintGoogleDebug --continue --no-configuration-cache` passes with the bundled Android Studio JBR 21 and G1GC (`BUILD SUCCESSFUL` in 10m13s; 1,951 actionable tasks). The resulting test XML set contains 2,439 tests with zero failures or errors. This verifies local compilation, formatting, static checks, tests, KMP smoke compilation, lint, and debug packaging; it does not establish Play release readiness.
- **Android Packaging Status (2026-07-17):** F-Droid arm64 debug packaging pins `mil.nga.geopackage:geopackage-android` 6.7.5 so its SQLite native dependency supports Android's 16 KB page-size requirement. The APK passes `zipalign -c -P 16`, and the packaged arm64 native libraries were audited for 16 KB `PT_LOAD` alignment. Preserve this override until every transitive flavor resolves an equally compatible or newer upstream dependency.
- **Parent Interop Status (2026-07-17):** Debug MeshLink trusts only `com.ntsocial.android.debug` when the debuggable host and parent share a signer digest; release pairing remains package- and signer-pinned. The fixed MeshLink and NTsocial parent debug builds were clean-installed on three Android 16 phones. Provider status, explicit parent-to-MeshLink launch, lifecycle switching, primary screens, and parent text-entry/sync paths completed without crash, ANR, fatal log, or trust failure. The no-radio status correctly remained "preparing the NTsocial radio channel."
- **Play Release Status (2026-07-17):** No Play-uploadable AAB has been validated or added to Git. The local Google release trial reached R8, then the production mapping upload rejected the repository's dummy Firebase configuration. A real upload keystore and the authorized production Google/Firebase/DataDog configuration are required for the unchanged official release workflow. Never present device-tested debug APKs or the build-script fallback debug signature as Play-ready, and never disable production mapping uploads merely to claim release success.
...
- **Three-Phone No-Radio Validation (2026-07-17):** Fixed debug APKs were clean-installed on `SM-S9080`, `SM-S9280`, and `OPPO CPH2695`, all Android 16/API 36 arm64 devices. Connections, Messages, Nodes, Map, MeshCore, Settings, onboarding, parent Provider status, explicit App launch, repeated foreground/background switching, and relevant log windows passed. English-keyboard text entry was re-tested successfully, and parent-local/BLE message synchronization was observed across phones. Because no Meshtastic or MeshCore hardware node was available, no current-run radio connection, LoRa transmission, RF reception, remote receipt, or MeshCore transport claim may be inferred from this result.
</architecture_boundaries>
...
- **Android 16 Native Compatibility:** Keep the 6.7.5 GeoPackage override (or a verified newer equivalent) for the F-Droid osmdroid path. After changing any native dependency, audit the produced arm64 APK with `zipalign -c -P 16` and verify each packaged ELF's `PT_LOAD` alignment before device delivery.
