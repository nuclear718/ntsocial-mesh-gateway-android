 ...
 *
 * The default flavor is configurable via the `meshlink.defaultMarketplace` Gradle property (defaults to the
 * [MeshLinkFlavor] entry marked `default = true`, which is `google`).
 */
internal fun Project.configureAndroidMarketplaceFallback() {
    val defaultMarketplace =
        providers
            .gradleProperty("meshlink.defaultMarketplace")
            .orElse(MeshLinkFlavor.entries.first { it.default }.name)
            .get()

    // AGP publishes the typed ProductFlavorAttr on flavored variant configurations.
    val marketplaceAttr = ProductFlavorAttr.of(MeshLinkFlavor.fdroid.dimension.name)
